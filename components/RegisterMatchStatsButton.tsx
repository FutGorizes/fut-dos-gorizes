...
export default function RegisterMatchStatsButton({
  ...
  async function salvar() {
    try {
      ...
      if (!response.ok) {
        toast.error(data.message || "Erro ao salvar estatísticas");

        return;
      }

      toast.success("Estatísticas registradas");
            ...
            {estatisticas.map((j, index) => (
              <div
                ...
                className="
                    ...
                    "
              >
                <p className="mb-3 font-black">{j.nome}</p>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="mb-2 text-sm font-bold">Gols</p>

                    <div className="flex items-center justify-between">
                      <Button
                        size="icon"
                        variant="outline"
                        onClick={() => alterarValor(index, "gols", -1)}
                      >
                        <Minus size={16} />
                      </Button>

                      <span className="font-black">{j.gols}</span>
                  ...
                  <div>
                    <p className="mb-2 text-sm font-bold">Assistências</p>

                    <div className="flex items-center justify-between">
                      <Button
                        size="icon"
                        variant="outline"
                        onClick={() => alterarValor(index, "assistencias", -1)}
                      >
                        <Minus size={16} />
                      </Button>

                      <span className="font-black">{j.assistencias}</span>
